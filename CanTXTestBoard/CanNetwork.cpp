...
START_INIT:
    ...
    // _CAN->mcp2515_setCANCTRL_Mode(MODE_ONESHOT);
}
...
void CanNetwork::setTime(uint32_t unix_time_seconds) {
  // Set offset to unix time (converted to ms) - current millis
  millis_absolute_time_offset = (unix_time_seconds * 1000) - millis();
}
bool CanNetwork::send(CanPacket * packet)
{
...
CanPacket CanNetwork::receive()
{
    ...
    {
        received.timestamp = millis_absolute_time_offset + millis();
